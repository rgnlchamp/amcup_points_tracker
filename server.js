...
// ACRS Point Scale (1st to 40th place)
const POINT_SCALE = [
    60, 54, 48, 43, 40, 37, 34, 32, 30, 28,  // 1st-10th
    27, 26, 25, 24, 23, 22, 21, 20, 19, 18,  // 11th-20th
    17, 16, 15, 14, 13, 12, 11, 10, 9, 8,    // 21st-30th
    7, 6, 5, 4, 3, 2, 1, 1, 1, 1             // 31st-40th
];
...
async function scrapeRaceResults(eventId) {
    try {
        ...
        console.log(`Fetching event overview: ${eventUrl}`);
        ...
        for (const raceUrl of raceUrls) {
            try {
                ...
                if (!distance) {
                    console.log(`WARNING: Could not detect distance from title: ${pageTitle}`);
                    continue;
                }
        ...
        console.log(`Total USA results found: ${allResults.length}`);
        return allResults;
...
function classifySkater(category) {
    ...
    if (JUNIOR_CATEGORIES.includes(category)) {
...
function processRaceData(raceResults, eventName) {
    ...
    raceResults.forEach(result => {
        ...
        if (!byDistanceGender[key]) {
            byDistanceGender[key] = [];
        }
    ...
    Object.keys(byDistanceGender).forEach(key => {
        ...
        sortedResults.forEach((result, index) => {
            ...
            const categories = classifySkater(result.category);
            ...
            if (categories.junior) {
                ...
                standings.junior[result.name].distances[distance][eventName] = points;
                standings.junior[result.name].totalPoints += points;
            }

            if (categories.master) {
                ...
                standings.master[result.name].distances[distance][eventName] = points;
                standings.master[result.name].totalPoints += points;
            }
        });
    });
...
function calculateCombinationStandings(allStandings) {
    const combinations = {
        overall: { sprint: [], longDistance: [] },
        junior: { sprint: [], longDistance: [] },
        master: { sprint: [], longDistance: [] }
    };
    ...
    ['overall', 'junior', 'master'].forEach(category => {
        ...
        Object.values(categoryStandings).forEach(skater => {
            ...
            if (sprintPoints > 0) {
                combinations[category].sprint.push({
                    name: skater.name,
            ...
            if (ldPoints > 0) {
                combinations[category].longDistance.push({
                    name: skater.name,
                    category: skater.category,
                    points: ldPoints,
                    details: details
        ...
        combinations[category].sprint.sort((a, b) => b.points - a.points);
        combinations[category].longDistance.sort((a, b) => b.points - a.points);
    });
...
app.post('/api/scrape-event', async (req, res) => {
    try {
        ...
        const combinations = calculateCombinationStandings({ overall: standings.overall, junior: standings.junior, master: standings.master });
...
app.post('/api/generate-pdf', async (req, res) => {
    try {
        const { standings, combinations } = req.body;
